...
const DailyReport = () => { 
    ...
    useEffect(() => {
      ...
      function getMonthlyReport() {
        axios
          .get("http://localhost/umap-server/dailyReport.php")
        ...
        <div ref={printRef} style={{height:window.innerHeight,width:'100%'}}>
            <div style={{width:"80%",margin:80}}>
                <div style={{width:"100%",backgroundColor:"#fcb040",overflow:"hidden"}}>
                    <h1 style={{color:"white",padding:20}}>Daily Report
                    
                    <p style={{color:"white",fontSize:12}}>{` ${searchlog.slice(0,1).map((e)=>e.today)}`}</p>
                    </h1>
